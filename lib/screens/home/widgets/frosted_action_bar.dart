// lib/screens/home/widgets/frosted_action_bar.dart
          ...
          child: twoRows
              ? Column(
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(child: _SolidButton(icon: Icons.price_change, label: sending ? 'Estimating' : 'Estimate', onTap: (sending || !canEstimate) ? null : onEstimate, busy: sending)),
                        const SizedBox(width: 8),
...
class _SolidButton extends StatelessWidget {
  ...
  const _SolidButton({required this.icon, required this.label, required this.onTap, required this.busy});
  ...
  Widget build(BuildContext context) {
    ...
    return ElevatedButton(
      ...
      child: Row(
        ...
        children: [
          if (busy) const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) else Icon(icon, size: 20),
          const SizedBox(width: 6),
          Flexible(child: Text(label, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, softWrap: true, style: const TextStyle(fontWeight: FontWeight.w600))),
        ],
      ),
    );
  }
